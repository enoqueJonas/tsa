...
const valuesAndTypesBlocks: LessonBlock[] = [
    ...
    { type: "paragraph", text: "Programs receive external representations and turn them into values that carry meaning. Before the application has HTTP endpoints, serializers or database models, you need a precise mental model of what Python is manipulating: objects, names that reference those objects, and operations allowed by their types." },
    { type: "callout", tone: "the application", title: "the application connection", body: "A future service-registration request may arrive as JSON containing a service name, lifecycle state, criticality, owning team and optional technical owner. Those bytes do not become trustworthy domain data merely because Python can represent them. Parsing, type conversion and validation are different responsibilities." },
    ...
    { type: "callout", tone: "note", title: "A type is not a domain rule", body: "Python can represent many numeric values, but the application still has to decide what values are valid. A review interval of -30 is a valid integer and still an invalid business value." },
    ...
    { type: "paragraph", text: "True and False represent boolean state. None represents the absence of a value, not an empty string, zero or False. Those distinctions matter in the application: 'no technical owner assigned', 'zero open incidents', and 'not production-facing' are different facts." },
    ...
    { type: "callout", tone: "the application", title: "Think ahead to the API", body: "When Django REST Framework arrives, serializers will help parse and validate incoming service-registry representations. The framework does not replace this mental model. It gives us a boundary at which to apply it consistently." },

    { type: "heading", id: "assignment", text: "Assignment" },
    { type: "list", ordered: true, items: ["Open a Python REPL and create representative application values: service slug, display name, lifecycle, criticality, review interval, production flag and optional technical owner. Inspect each with type() and repr().", "Create two equal dependency lists and prove the difference between == and is. Then create an alias and mutate the list through the alias.", "Parse the strings '90', '090' and 'quarterly' as integers. Record what succeeds, what fails and what that implies for API validation.", "Write down three the application examples where None, an empty string and zero would mean different things.", "Read the linked Python tutorial sections as reinforcement, not as a substitute for the experiments above."] },
...
export const programmingWithPythonDeepLessons: Lesson[] = [
    ...
    deepLesson("Errors, Exceptions and Defensive Programming", "Failures are part of a function contract; broad catching can destroy evidence.", "Design explicit failure behavior for a application service operation.", ["Choose two invalid conditions.", "Define rejection points.", "Translate exceptions only at abstraction boundaries.", "Demonstrate why broad catching harms diagnosis.", "Record caller-facing failure contracts."], "When should an exception propagate, be wrapped or become a normal domain result?"),
    deepLesson("Object-Oriented Programming", "Objects are useful when data and behavior share invariants and lifecycle; not every noun deserves a class.", "Model one the application concept as an object and compare it with a simpler representation.", ["Identify state plus behavior.", "Protect invariants.", "Use composition.", "Build a functional alternative.", "Compare approaches."], "When is a class unnecessary? How can inheritance increase coupling?"),
    deepLesson("Comprehensions, Iterators and Pythonic Tools", "Python offers concise traversal tools, but concision is not automatically clarity.", "Transform application service-registry data using expressive iteration while preserving readability.", ["Start with an explicit loop.", "Compare a comprehension.", "Use a generator.", "Demonstrate iterator consumption.", "Use any/all or sorting clearly."], "When is a comprehension worse than a loop?"),
    ...
    { id: "programming-with-python-lab-build-a-small-python-service-core", title: "Lab: Build a Small Python Service Core", activities: [
        { id: "programming-with-python-lab-build-a-small-python-service-core-brief", title: "Service Core Design Brief", estimatedMinutes: 20, content: { type: "reading", body: "Build a framework-free Python core around a narrow slice of the application's service-registry domain: services, teams, ownership, lifecycle or dependencies. The goal is to expose decisions about validation, state, errors, modules and interfaces before Django arrives." } },
        { id: "programming-with-python-lab-build-a-small-python-service-core-build", title: "Build the Service Core", estimatedMinutes: 150, content: { type: "practical", objective: "Build a credible Python service-registry core that becomes the conceptual starting point for the application API.", scenario: "the application will later become a Django/DRF service. Model a small slice of service ownership, lifecycle or dependency behavior with plain Python first.", instructions: ["Define a narrow application domain problem and 3–5 rules.", "Design module boundaries.", "Implement domain behavior.", "Reject invalid state deliberately.", "Add type annotations.", "Demonstrate happy, boundary and failure paths.", "Recreate in a clean environment.", "Debug one deliberate defect.", "Record what should remain framework-independent."], deliverables: ["Runnable service-registry core package", "Domain rules", "Dependency declaration", "Failure evidence", "Debugging log", "Framework-independence note"], completionCriteria: ["Real application domain behavior is demonstrated.", "Boundaries can be explained.", "Failures are observable.", "A clean environment reproduces the project."] } },
        { id: "programming-with-python-lab-build-a-small-python-service-core-review", title: "Python Module Review", estimatedMinutes: 20, content: { type: "reflection", prompt: "Which part demonstrates Python understanding? Which is over-engineered? Which the application assumption is dangerous? What would you refuse to redesign until later evidence exists?" } },
    ] },
];
