        ...
        activities: [
            ...
            },
            {
                ...
                content: {
                    type: "practical",
                    objective,
                    scenario: "Work in the framework-free the application Python workspace. Every useful artifact should be preserved because later lessons and the Django/DRF modules will build on this service-registry domain rather than restart from a toy project.",
                    instructions,
                    deliverables: ["Runnable Python evidence", "Short engineering note with at least one failure, edge case or trade-off"],
                    completionCriteria: ["The result runs and can be demonstrated.", "The learner can explain the Python behavior used.", "The work uses the locked application service-registry domain when a domain example is appropriate."],
                },
            },
            {
                id: `${id}-check`,
...
const environment = lesson(
    ...
    [
        ...
        { type: "callout", tone: "the application", title: "the application connection", body: "The same discipline will later matter in CI, Docker images and production. If the the application API only works because your laptop happens to contain the right package versions, the environment is not an engineering artifact yet." },
...
const valuesTypes = lesson(
    ...
    [
        ...
        { type: "callout", tone: "the application", title: "the application connection", body: "A future service-registration request may contain a service slug, criticality, lifecycle and technical-owner identifier. JSON can carry those values, but JSON does not decide whether they are valid application domain values." },
        { type: "heading", id: "outcomes", text: "Learning outcomes" },
...
const functions = lesson(
    ...
    [
        ...
        { type: "paragraph", text: "Functions are often introduced as a way to avoid repeating code. That is useful, but it is not the main reason we care about them in the application. A good function creates a small boundary: you can see what goes in, what comes back, what may change, and how it can fail. Give that boundary a clear name and callers no longer need to understand every line inside it." },
...
const collections = lesson(
    ...
    [
        ...
        { type: "paragraph", text: "Dictionaries make keyed lookup explicit but do not automatically enforce a schema. As the application grows, unrestricted dictionaries eventually become too weak for core domain concepts; later lessons will show when classes and database models provide stronger structure." },
        ...
        { type: "callout", tone: "the application", title: "Domain choice", body: "If duplicate dependencies are invalid, a set can help in memory—but later PostgreSQL must enforce the rule too. An in-memory structure is not a substitute for a persistence constraint." },
    ...
    "For each of list, tuple, set and dict, name one the application use case and the guarantee that makes the structure suitable."
);
...
const modules = lesson(
    ...
    [
        ...
        { type: "paragraph", text: "A Python package organizes importable code. Publishing a reusable internal distribution is a separate decision with versioning, ownership and compatibility consequences. We will not extract the application-common until a real reuse boundary exists." },
        { type: "resources", title: "Required reading", resources: [pythonTutorial, packagingGuide] },
    ],
    "Split the evolving the application core into coherent Python modules while keeping dependency direction understandable.",
...
const typeHints = lesson(
    ...
    "Add useful type annotations to the the application core and use a static checker to expose a real mismatch.",
...
const dependencies = lesson(
    ...
    [
        ...
        { type: "callout", tone: "the application", title: "Future internal distribution", body: "Later, genuinely reusable packages such as the application-common will be versioned and published through the internal Nexus/PyPI path. Do not manufacture that package now; first build enough real code to discover a reuse boundary." },
        { type: "resources", title: "Required reading", resources: [packagingGuide, venvDocs] },
    ],
    "Make the the application Python core reproducible from a clean environment and explain its dependency constraints.",
    ...
    activities: [
        {
            ...
            content: {
                type: "reading",
                body: "Build the framework-independent beginning of the application.",
                blocks: [
                    ...
                    { type: "callout", tone: "the application", title: "Do not build Django yet", body: "This lab is about Python domain behavior and package boundaries. Django/DRF arrives later as a framework around the appropriate parts of this core, not as the source of every business rule." },
