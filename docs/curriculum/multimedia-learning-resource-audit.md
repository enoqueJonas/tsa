...
| Operate DHCP Scopes and Reservations | RECOMMENDED VIDEO | DHCP server/scope demonstrations reviewed | No candidate promoted yet | Scope, lease, option and reservation behavior is easier to understand with a client/server sequence and live lease evidence. |
...
| Services, Processes and Event Logs | RECOMMENDED VIDEO | PowerShell/Event Viewer incident demos reviewed | No candidate promoted yet | A live Windows service failure traced through processes/services/event logs adds operational value. |
| NTFS and Share Permissions | RECOMMENDED VIDEO | NTFS/share effective-access demonstrations reviewed | No candidate promoted yet | Share+NTFS permission intersection is much easier to understand visually. |
| Remote Administration with PowerShell Remoting | RECOMMENDED VIDEO | WinRM/PowerShell remoting demonstrations reviewed | No candidate promoted yet | Seeing local versus remote execution, authentication and failure boundaries is valuable. |
| Active Directory, DNS and Domain Identity | RECOMMENDED VIDEO | Microsoft AD DS/DNS architecture explainers reviewed | No candidate promoted yet | Domain controller, DNS, users/groups/OUs and authentication relationships are highly visual. |
