# Multimedia Learning Resource Audit

This is the canonical multimedia matrix for the live TSA curriculum. Historical working sections and superseded decisions are preserved in `docs/curriculum/archive/multimedia-learning-resource-audit-history.md`.

The matrix is evidence-first: multimedia supports TSA-authored teaching and hands-on engineering; it does not replace labs, reviews, milestones, operational drills or defences.
...
## Architect — complete school multimedia audit
...
| Fundamentals — Quality Attributes and Architecture Characteristics | RECOMMENDED VIDEO | SEI quality-attribute scenario material reviewed | [SEI — Architecture Tradeoff Analysis Method](https://www.sei.cmu.edu/library/architecture-tradeoff-analysis-method-collection/) — use quality-attribute scenarios and trade-off analysis | Worked quality scenarios connect vague qualities to measurable architectural pressure. |
...
| Fundamentals — Architecture Fitness and Evolution | RECOMMENDED VIDEO | Thoughtworks evolutionary-architecture talks reviewed | [Thoughtworks — Evolutionary Architecture](https://www.thoughtworks.com/insights/articles/fitness-function-driven-development) — use fitness-function examples | Fitness-function/evolution examples can make continuous architecture concrete. |
...
| Styles — Hexagonal and Ports-and-Adapters Thinking | RECOMMENDED VIDEO | Alistair Cockburn/ports-and-adapters talks reviewed | [Alistair Cockburn — Hexagonal Architecture](https://alistair.cockburn.us/hexagonal-architecture/) — canonical ports/adapters diagrams | Boundary/port/adapter direction is highly visual. |
| Styles — Modular Monolith | RECOMMENDED VIDEO | modular-monolith talks reviewed | [Microsoft — Common web application architectures](https://learn.microsoft.com/en-us/dotnet/architecture/modern-web-apps-azure/common-web-application-architectures) — use monolithic/modular boundary diagrams | Module boundaries and deployment unity are easier to compare visually with services. |
...
| Styles — Microservices | RECOMMENDED VIDEO | Martin Fowler microservices material and practitioner talks reviewed | [Martin Fowler — Microservices](https://martinfowler.com/articles/microservices.html) — use service-boundary and distribution trade-offs | Service boundaries, independent deployment and distribution cost benefit from worked architecture examples. |
| Styles — Event-driven Architecture | RECOMMENDED VIDEO | AWS/event-driven architecture talks reviewed | [AWS — What is Event-Driven Architecture?](https://aws.amazon.com/event-driven-architecture/) — use producer/router/consumer diagrams | Producer/event/broker/consumer topology and temporal decoupling are visual. |
...
| Domain — Bounded Context Concepts | RECOMMENDED VIDEO | Martin Fowler bounded-context material and DDD talks reviewed | [Martin Fowler — Bounded Context](https://martinfowler.com/bliki/BoundedContext.html) — canonical context-boundary model | Context boundaries and language shifts are strongly diagrammatic. |
| Domain — Entities, Value Objects and Aggregates | RECOMMENDED VIDEO | DDD tactical-modeling talks reviewed | [Martin Fowler — Domain-Driven Design](https://martinfowler.com/bliki/DomainDrivenDesign.html) — use tactical/domain-boundary references | Worked object/aggregate examples help distinguish identity, value and consistency boundaries. |
| Domain — Invariants and Transaction Boundaries | RECOMMENDED VIDEO | aggregate/invariant talks reviewed | [Martin Fowler — Domain-Driven Design](https://martinfowler.com/bliki/DomainDrivenDesign.html) — use tactical/domain-boundary references | A transaction-boundary walkthrough can connect domain rules to persistence. |
...
| Modularity — Modules and Boundaries | RECOMMENDED VIDEO | modularity/boundary talks reviewed | [Microsoft — Common web application architectures](https://learn.microsoft.com/en-us/dotnet/architecture/modern-web-apps-azure/common-web-application-architectures) — use dependency and application-boundary diagrams | Dependency and ownership boundaries benefit from visual examples. |
...
| Modularity — Dependency Direction | RECOMMENDED VIDEO | dependency-rule/clean-architecture diagrams reviewed | [Microsoft — Common web application architectures](https://learn.microsoft.com/en-us/dotnet/architecture/modern-web-apps-azure/common-web-application-architectures) — use dependency and application-boundary diagrams | Direction and policy/mechanism boundaries are visual. |
...
| Modularity — Shared Libraries and Coupling | RECOMMENDED VIDEO | shared-library/platform dependency talks reviewed | [Microsoft — Common web application architectures](https://learn.microsoft.com/en-us/dotnet/architecture/modern-web-apps-azure/common-web-application-architectures) — use dependency and application-boundary diagrams | Version/coupling blast radius benefits from concrete examples. |
...
| Data — Data Ownership | RECOMMENDED VIDEO | data ownership/microservice data talks reviewed | [Microsoft — Data considerations for microservices](https://learn.microsoft.com/en-us/azure/architecture/microservices/design/data-considerations) — use ownership and consistency trade-offs | Ownership boundaries and cross-service data access benefit from diagrams. |
...
| Data — Transactional Consistency | RECOMMENDED VIDEO | transaction/consistency talks reviewed | [PostgreSQL — Transaction Isolation](https://www.postgresql.org/docs/current/transaction-iso.html) plus distributed-boundary discussion — ground local transaction semantics first | Transaction scope and cross-boundary consistency are temporal and visual. |
| Data — Data Duplication and Derived Views | RECOMMENDED VIDEO | CQRS/read-model material reviewed | [Microsoft — CQRS pattern](https://learn.microsoft.com/en-us/azure/architecture/patterns/cqrs) — use write/read model flow | Source-of-truth versus derived-copy flow benefits from visualization. |
...
| Data — Read Models | RECOMMENDED VIDEO | CQRS/read-model talks reviewed | [Microsoft — CQRS pattern](https://learn.microsoft.com/en-us/azure/architecture/patterns/cqrs) — use write/read model flow | Write model→event/data→read model flow is visual. |
| Data — Database-per-service Trade-offs | RECOMMENDED VIDEO | microservice data architecture talks reviewed | [Microsoft — Data considerations for microservices](https://learn.microsoft.com/en-us/azure/architecture/microservices/design/data-considerations) — use ownership and consistency trade-offs | Ownership gains versus distributed consistency/query costs benefit from worked examples. |
| Data — Data Migration and Evolution | RECOMMENDED VIDEO | expand-contract/online migration talks reviewed | [Martin Fowler — Parallel Change](https://martinfowler.com/bliki/ParallelChange.html) — expand/migrate/contract sequence | Coexistence/backfill/cutover/contract sequence is a strong timeline use case. |
...
| Integration — Integration Styles | RECOMMENDED VIDEO | integration-pattern talks reviewed | [Enterprise Integration Patterns](https://www.enterpriseintegrationpatterns.com/patterns/messaging/) — use messaging topology catalogue | Request/reply, messaging, files and events benefit from side-by-side topology diagrams. |
...
| Integration — Queues and Publish-Subscribe | RECOMMENDED VIDEO | queue/pub-sub architecture explainers reviewed | [AWS — Publish-subscribe messaging](https://docs.aws.amazon.com/prescriptive-guidance/latest/cloud-design-patterns/publish-subscribe.html) — use publisher/topic/subscriber topology | Fan-out and competing-consumer semantics are visual. |
| Integration — Events and Commands | RECOMMENDED VIDEO | event-vs-command talks reviewed | [Martin Fowler — What do you mean by Event-Driven?](https://martinfowler.com/articles/201701-event-driven.html) — distinguish event notification/state transfer/event sourcing | Intent versus fact and coupling consequences benefit from examples. |
...
| Scale — Load Distribution | RECOMMENDED VIDEO | load-balancing algorithms/demos reviewed | [AWS — Elastic Load Balancing](https://docs.aws.amazon.com/elasticloadbalancing/latest/userguide/what-is-load-balancing.html) — use request distribution architecture | Request distribution across instances is highly visual. |
...
| Scale — Replication Concepts | RECOMMENDED VIDEO | database replication animations reviewed | [PostgreSQL — High Availability, Load Balancing, and Replication](https://www.postgresql.org/docs/current/high-availability.html) — canonical primary/standby models | Primary/replica write/read/failure flows are visual. |
| Scale — Partitioning Concepts | RECOMMENDED VIDEO | sharding/partitioning visual explainers reviewed | [PostgreSQL — Table Partitioning](https://www.postgresql.org/docs/current/ddl-partitioning.html) plus TSA distributed-partitioning diagrams | Key→partition placement and rebalance are spatial. |
| Scale — Consistency and Availability Trade-offs | RECOMMENDED VIDEO | distributed consistency/CAP talks reviewed | [Martin Kleppmann — Designing Data-Intensive Applications resources](https://dataintensive.net/) plus TSA partition scenarios — avoid slogan-only CAP treatment | Network partition scenarios need careful visual treatment; avoid slogan-only CAP videos. |
...
| Resilience Architecture — Timeouts | RECOMMENDED VIDEO | timeout/deadline demos reviewed | [AWS Builders' Library — Timeouts, retries and backoff with jitter](https://aws.amazon.com/builders-library/timeouts-retries-and-backoff-with-jitter/) — production failure/amplification examples | Caller waiting and resource occupancy are temporal. |
| Resilience Architecture — Retries and Retry Storms | RECOMMENDED VIDEO | retry amplification visualizations reviewed | [AWS Builders' Library — Timeouts, retries and backoff with jitter](https://aws.amazon.com/builders-library/timeouts-retries-and-backoff-with-jitter/) — production failure/amplification examples | Amplification is ideal for animation. |
| Resilience Architecture — Circuit Breakers | RECOMMENDED VIDEO | circuit-breaker state-machine demos reviewed | [Microsoft — Circuit Breaker pattern](https://learn.microsoft.com/en-us/azure/architecture/patterns/circuit-breaker) — state-transition diagram | Closed/open/half-open transitions are inherently visual. |
| Resilience Architecture — Bulkheads | RECOMMENDED VIDEO | bulkhead/isolation diagrams reviewed | [Microsoft — Bulkhead pattern](https://learn.microsoft.com/en-us/azure/architecture/patterns/bulkhead) — resource-isolation diagrams | Resource-pool isolation benefits from diagrams. |
...
| Evaluation — Scenario-based Architecture Evaluation | RECOMMENDED VIDEO | ATAM/SEI architecture-evaluation material reviewed | [SEI — Architecture Tradeoff Analysis Method](https://www.sei.cmu.edu/library/architecture-tradeoff-analysis-method-collection/) — use quality-attribute scenarios and trade-off analysis | Scenario walkthroughs show how quality risks emerge from architectural choices. |
| Evaluation — Fitness Functions | RECOMMENDED VIDEO | evolutionary-architecture fitness-function talks reviewed | [Thoughtworks — Evolutionary Architecture](https://www.thoughtworks.com/insights/articles/fitness-function-driven-development) — use fitness-function examples | Automated architecture constraints benefit from concrete demonstrations. |
...
## Technical Steward — complete school multimedia audit

| Path / lesson | Decision | Candidates checked | Selection / segment | Reason |
| --- | --- | --- | --- | --- |
| Governance — Governance vs Management | RECOMMENDED VIDEO | COBIT governance-vs-management explainers reviewed | [ISACA — COBIT](https://www.isaca.org/resources/cobit) — use the governance-system and governance-vs-management model | A concise governance/management responsibility model can anchor the school. |
...
| Governance — Policies, Standards, Procedures and Guidelines | RECOMMENDED VIDEO | policy/standard/procedure governance explainers reviewed | [NIST — Cybersecurity Framework 2.0](https://www.nist.gov/cyberframework) — use Govern outcomes plus TSA side-by-side artifact examples | Side-by-side examples help distinguish artifact purpose and mandatory strength. |
...
| Technology Governance — COBIT Concepts | RECOMMENDED VIDEO | ISACA COBIT overview material reviewed | [ISACA — COBIT](https://www.isaca.org/resources/cobit) — official framework overview | A concise official overview can orient governance objectives without teaching the whole framework. |
| Technology Governance — ITIL and Service-management Concepts | RECOMMENDED VIDEO | PeopleCert/ITIL service-management overviews reviewed | [PeopleCert — ITIL](https://www.peoplecert.org/browse-certifications/it-governance-and-service-management/ITIL-1) — use service/value-system overview only | Value/service lifecycle concepts benefit from a concise framework overview. |
...
| Technology Governance — NIST Framework Landscape | RECOMMENDED VIDEO | NIST CSF/RMF/SSDF overview material reviewed | [NIST — Cybersecurity and Privacy Reference Tool](https://csrc.nist.gov/projects/cprt) plus CSF/RMF/SSDF framework pages — compare scope, not control IDs | Seeing framework scopes side by side helps prevent checklist conflation. |
...
| Risk — Inherent and Residual Risk | RECOMMENDED VIDEO | risk/control visual explainers reviewed | [NIST SP 800-30 Rev. 1 — Guide for Conducting Risk Assessments](https://csrc.nist.gov/pubs/sp/800/30/r1/final) — use threat/likelihood/impact/control relationship | Before-controls→controls→residual exposure is well suited to a diagram. |
...
| Technical Leadership — Influence Without Authority | RECOMMENDED VIDEO | technical-leadership influence talks reviewed | [StaffEng — Stories of Staff-plus Engineers](https://staffeng.com/stories/) — select one cross-team influence case | Observed communication/decision scenarios are valuable because the capability is interpersonal rather than tool-based. |
...
| Technical Leadership — Technical Communication | RECOMMENDED VIDEO | technical communication/review talks reviewed | [Google Engineering Practices — Code Review](https://google.github.io/eng-practices/review/) — use review communication and standards examples | Seeing a concise technical explanation and challenge exchange is useful. |
...
| Controls — Control Effectiveness | RECOMMENDED VIDEO | control design-vs-operating-effectiveness material reviewed | [NIST SP 800-53A Rev. 5](https://csrc.nist.gov/pubs/sp/800/53/a/r5/final) — use assessment-objective/evidence model | Side-by-side evidence examples help distinguish intended design from operation. |
| Controls — Evidence | NO VIDEO | — | — | The learner must identify and preserve actual operating evidence. |
| Controls — Control Testing | RECOMMENDED VIDEO | audit/control-testing walkthroughs reviewed | [NIST SP 800-53A Rev. 5](https://csrc.nist.gov/pubs/sp/800/53/a/r5/final) — examine/interview/test assessment methods | Sampling/reperformance/inspection are useful to see applied to a concrete control. |
...
| Third-party — End-of-life Technology | RECOMMENDED VIDEO | technology lifecycle/EOL case studies reviewed | [CISA — Known Exploited Vulnerabilities Catalog](https://www.cisa.gov/known-exploited-vulnerabilities-catalog) plus vendor lifecycle notices — connect unsupported technology to exposure and migration urgency | A lifecycle timeline with support/patch/migration consequences is useful. |
| Third-party — Exit and Continuity Planning | RECOMMENDED VIDEO | cloud/SaaS exit-strategy talks reviewed | [NIST SP 800-161 Rev. 1](https://csrc.nist.gov/pubs/sp/800/161/r1/final) — use supply-chain lifecycle and dependency-risk context for exit planning | Data export, replacement, cutover and continuity are strong sequence/topology topics. |
