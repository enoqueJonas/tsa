...
| Messaging — Acknowledgements, Retries and Delivery Semantics | RECOMMENDED VIDEO | RabbitMQ reliability material reviewed | [AWS Builders' Library — Timeouts, retries and backoff with jitter](https://aws.amazon.com/builders-library/timeouts-retries-and-backoff-with-jitter/) — production failure/amplification examples | Delivery/ack/redelivery timelines benefit from live demonstration. |
...
| API/Integration — Schema and Contract Validation | RECOMMENDED VIDEO | Contract-testing/Pact demonstrations reviewed | [Confluent — Schema Evolution and Compatibility](https://docs.confluent.io/platform/current/schema-registry/fundamentals/schema-evolution.html) — use compatibility modes and producer/consumer evolution | Provider/consumer expectation flow is visual. |
...
| Automation — Retries and Flaky-test Risk | RECOMMENDED VIDEO | Flaky-test/retry talks reviewed | [AWS Builders' Library — Timeouts, retries and backoff with jitter](https://aws.amazon.com/builders-library/timeouts-retries-and-backoff-with-jitter/) — production failure/amplification examples | A repeated intermittent failure makes masking risk concrete. |
...
| Resilience — Timeouts | RECOMMENDED VIDEO | timeout/failure-timeline demos reviewed | [AWS Builders' Library — Timeouts, retries and backoff with jitter](https://aws.amazon.com/builders-library/timeouts-retries-and-backoff-with-jitter/) — production failure/amplification examples | Bounded waiting is temporal. |
| Resilience — Retries | RECOMMENDED VIDEO | retry failure demos reviewed | [AWS Builders' Library — Timeouts, retries and backoff with jitter](https://aws.amazon.com/builders-library/timeouts-retries-and-backoff-with-jitter/) — production failure/amplification examples | Retry behavior under transient failure is temporal. |
...
| Resilience — Circuit Breakers | RECOMMENDED VIDEO | circuit-breaker state-machine demos reviewed | [Microsoft — Circuit Breaker pattern](https://learn.microsoft.com/en-us/azure/architecture/patterns/circuit-breaker) — state-transition diagram | Closed/open/half-open states are visual. |
...
| Resilience — Queues and Backpressure | RECOMMENDED VIDEO | backpressure/queue demos reviewed | [Reactive Manifesto](https://www.reactivemanifesto.org/) plus Reactive Streams concepts — use demand/back-pressure relationship | Producer/consumer imbalance and queue growth are temporal. |
