...
function activities(path: LearningPath) {
...
export function AcademyDashboard({ onOpenCurriculum, onOpenPath }: AcademyDashboardProps) {
  ...
  const entries = journey.schools.flatMap((school) => school.paths.map((path) => ({ school, path })));
  const last = lastVisited();
  const lastEntry = last ? entries.find(({ path }) => path.id === last.pathId) : undefined;
  const completedIds = new Set(Object.values(records).flatMap((record) => record.completedActivityIds));
  const totalActivities = entries.reduce((sum, { path }) => sum + activities(path).length, 0);
  const completedActivities = entries.reduce((sum, { path }) => sum + activities(path).filter((activity) => completedIds.has(activity.id)).length, 0);
  const overallPercent = totalActivities ? Math.round((completedActivities / totalActivities) * 100) : 0;
  const completedModules = entries.filter(({ path }) => pathStatus(path) === "completed").length;
  const inProgressModules = entries.filter(({ path }) => pathStatus(path) === "in-progress").length;
  const completedSchools = journey.schools.filter((school) => school.paths.every((path) => pathStatus(path) === "completed")).length;
  const totalMinutes = entries.reduce((sum, { path }) => sum + activities(path).reduce((value, activity) => value + activity.estimatedMinutes, 0), 0);
  const completedMinutes = entries.reduce((sum, { path }) => sum + activities(path).filter((activity) => completedIds.has(activity.id)).reduce((value, activity) => value + activity.estimatedMinutes, 0), 0);
  const recent = Object.values(records).sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)).slice(0, 4);
    ...
    <section className={ready ? "opacity-100" : "opacity-70"}>
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.55fr)_minmax(320px,0.75fr)]">
        ...
        <article className="rounded-3xl border border-zinc-200 bg-white p-8 shadow-sm">
          ...
          <div className="mt-8 grid grid-cols-2 gap-4 border-t border-zinc-100 pt-6">
            <div><p className="text-2xl font-semibold">{completedModules}</p><p className="mt-1 text-sm text-zinc-500">Modules completed</p></div>
            <div><p className="text-2xl font-semibold">{completedSchools}</p><p className="mt-1 text-sm text-zinc-500">Schools completed</p></div>
            <div><p className="text-2xl font-semibold">{inProgressModules}</p><p className="mt-1 text-sm text-zinc-500">In progress</p></div>
            <div><p className="text-2xl font-semibold">{formatMinutes(completedMinutes)}</p><p className="mt-1 text-sm text-zinc-500">Learning completed</p></div>
          </div>
        </article>
      </div>
      ...
      <div className="mt-8 grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(340px,0.65fr)]">
        <article className="rounded-3xl border border-zinc-200 bg-white p-7 shadow-sm">
          <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-400">The journey</p><h2 className="mt-2 text-2xl font-semibold">12 schools. One apprenticeship.</h2></div><button type="button" onClick={onOpenCurriculum} className="text-sm font-semibold text-blue-700 hover:text-blue-900">View curriculum →</button></div>
          <div className="mt-6 space-y-2">{journey.schools.map((school, index) => {
            const schoolActivities = school.paths.flatMap((path) => activities(path));
            const completed = schoolActivities.filter((activity) => completedIds.has(activity.id)).length;
            ...
            const firstPath = school.paths[0];
            return <button key={school.id} type="button" disabled={!firstPath} onClick={() => firstPath && onOpenPath(firstPath, school)} className="grid w-full grid-cols-[38px_minmax(0,1fr)_90px] items-center gap-4 rounded-2xl px-3 py-3 text-left transition hover:bg-zinc-50"><span className="text-sm tabular-nums text-zinc-400">{String(index + 1).padStart(2, "0")}</span><div><div className="flex items-center justify-between gap-4"><span className="font-medium">{school.title}</span><span className="text-xs text-zinc-500">{status}</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-zinc-100"><div className="h-full rounded-full bg-zinc-900" style={{ width: `${percent}%` }} /></div></div><span className="text-right text-sm tabular-nums text-zinc-500">{percent}%</span></button>;
          })}</div>
        </article>

        <div className="space-y-6">
          <article className="rounded-3xl border border-zinc-200 bg-white p-7 shadow-sm"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-400">Learning time</p><p className="mt-3 text-3xl font-semibold">{formatMinutes(completedMinutes)}</p><p className="mt-2 text-sm text-zinc-500">of approximately {formatMinutes(totalMinutes)} authored learning time completed.</p></article>
          <article className="rounded-3xl border border-zinc-200 bg-white p-7 shadow-sm"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-400">Recent activity</p><div className="mt-5 space-y-4">{recent.length ? recent.map((record) => { const entry = entries.find(({ path }) => path.id === record.pathId); if (!entry) return null; return <button key={record.pathId} type="button" onClick={() => onOpenPath(entry.path, entry.school)} className="block w-full text-left"><p className="text-sm font-medium">{entry.path.title}</p><p className="mt-1 text-xs text-zinc-500">{entry.school.title} · {pathPercent(entry.path)}%</p></button>; }) : <p className="text-sm text-zinc-500">No learning activity yet.</p>}</div></article>
        </div>
      </div>
    </section>
  );
